...
export default function SidebarRight({ isOpenSidebarRight, toggleSidebarRight, pages, currentPage, setCurrentPage }) {
    ...
    const [currentImageFit, setCurrentImageFit] = useState(0);
    const [currentProcessBarStyle, setCurrentProcessBarStyle] = useState(0);
    console.log(currentDisplayStyles);
    console.log(pageDisplayStyles[currentDisplayStyles].label);
    console.log(pageDisplayStyles[currentDisplayStyles].icon);
        ...
        <div className={`sidebar-right select-none fixed top-0 right-0 bg-[--background] p-[16px] w-[311px] h-full flex flex-col gap-2 ${!isOpenSidebarRight ? 'hidden' : ''}`}>
            ...
            <div className="page-route flex gap-1 ">
                ...
                >
                    ...
                    <div className={` rounded text-base w-full h-[384px] overflow-y-scroll bg-[--background-chapter] absolute left-0 top-14 z-10 mt-1 ${!switchDropdownPage ? 'hidden' : 'block'}`}

                    >
                        <ul className="">
                            {pages.map((page, index) => {
                                return (
                                    <li key={index} className="py-2 px-4 hover:bg-[--color-pink-purple] rounded cursor-pointer" onClick={() => {
