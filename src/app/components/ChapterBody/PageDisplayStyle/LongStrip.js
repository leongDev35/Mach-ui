...
export default function LongStrip({setFlagScroll, containerListPageRef,chapter, currentPage, setCurrentPage, pageRefs, toggleSidebarRight, currentImageFit, pages }) {
        ...
        <>
          {chapter.pages.map((page, index) => {
            return (
              <div
                key={index}
                ref={(el) => (pageRefs.current[index] = el)} // Gán ref cho tất cả chế độ
                className={`relative mt-1 ${
                  currentImageFit === 0 ? "w-full h-auto" : //! fit width
              ...
              >
                ...
                {/* Hình ảnh hiển thị */}
                <img
                  className={`object-contain max-w-full mt-1 ${
                    currentImageFit === 0 ? "w-full h-auto" :
